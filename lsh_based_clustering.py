...
class LSHBasedCluster:
    ...
    def clustering_in_chunks(self, resonable_chunk=0.5):
        """
        Second phase of the algorithm. Executes the LSH based algorithm in respect to each chunks alone.
    ...
    def results_str(self, metric=None):
        ...
        """
        time_st = time.time()
        size = len(dict(filter(lambda elem: len(elem[1]) >= 1, self.C_til.items())))
        if size == 0: return
...
class Accrcy(Metric):
    ...
    @timeit
    def print(self):
        results_dict = {gamma: self.calc(gamma) / self.size for gamma in [0.6, 0.7, 0.8, 0.9, 0.95, 0.99, 1.0]}
        return '\n'.join([("{}: {}".format(key, value)) for key, value in results_dict.items()])
    ...
    def calc(self, gamma):
        ...
        self.cnt_falsepos_mistake = 0
        self.cnt_notsufficientlybig_mistake = 0
        for i in self.C_til.keys():
            if len(self.C_til[i]) == 0:
                continue
            acrcy += self.comp_clstrs(self.C_til[i], self.C_dict[rep_in_C(self.all_reads[self.C_til[i][0]], self.C_reps)], gamma)
        info("gamma={}: {} ignored because of false positives ({} false positives in total). {} not sufficiently big. {} too big"
              .format(gamma, self.cnt_falsepos_mistake, self.cnt_falsepos, self.cnt_notsufficientlybig_mistake, self.cnt_toobig))
        return acrcy

    def comp_clstrs(self, alg_clstr, org_clstr, gamma):
        true_positives = 0
        max_false = 0
        ...
        for i in range(0, len(alg_clstr)):
            ...
            for j in range(0, len(org_clstr)):
                if self.all_reads[alg_clstr[i]] == self.all_reads[org_clstr[j]]:
                    flg_exist = 1
                    true_positives += 1
                    break
            if flg_exist == 0:
