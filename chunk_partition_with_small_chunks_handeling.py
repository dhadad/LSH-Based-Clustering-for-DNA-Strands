...
class LSHCluster:
    ...
    def chunk_paritioning(self, repeats=28):
        """
        kt = 5
        Lt= 64
        ...
        small_chunks = [idx for idx in range(len(self.all_reads)) if len(self.chunks[idx]) >= 1 and len(self.chunks[idx]) < SMALL_CHUNK * avg_chunk]
        print("-INFO: (common sub-string) average chunk size: {}, number of chunks smaller than {} is {} (before handeling small chunks)".format(avg_chunk, SMALL_CHUNK * avg_chunk, len(small_chunks)))
        cntr = 0
        ref = small_chunks[0]
        for small_chunk_rep in small_chunks:
            if cntr > avg_chunk:
                ...
                if p1 != p2:
                    cntr += len(self.chunks[p2])
                    center, merged = min(p1, p2), max(p1, p2)
                    self.chunks[center].extend(self.chunks[merged])
                    self.chunks[merged] = []
                    self.chunk_parent[merged] = center
                    ref = center
        print("-INFO: (common sub-string) handeling small chunks {} took: {}".format(itr, time.time() - time_itr))
        self.duration += time.time() - time_start
