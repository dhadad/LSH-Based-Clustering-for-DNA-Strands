...
from lsh_based_clustering import *
...
class dnaSimulator(QMainWindow, dnaSimulator_ui2.Ui_dnaSimulator):
    def __init__(self):
        ...
        self.lsh_q = 6
        self.lsh_sig_len = 3
        self.lsh_perm_num = 40
        self.lsh_iters = 32
        self.lsh_distance_threshold=12
        ...
        self.lsh_q_spinBox.textChanged.connect(self.set_lsh_q)
        self.lsh_sig_len_spinBox.textChanged.connect(self.set_lsh_sig_len)
        self.lsh_perm_num_spinBox.textChanged.connect(self.set_lsh_perm_num)
        self.lsh_iters_spinBox.textChanged.connect(self.set_lsh_iters)
        self.lsh_distance_threshold_spinBox.textChanged.connect(self.set_lsh_distance_threshold)
        ...
        self.clustering_listWidget.addItem('LSH Based Algorithm')
        ...
        self.stackedWidget.addWidget(self.lsh_based_clustering_settings_verticalWidget)
        ...
        self.lsh_technology_comboBox.addItem('Twist Bioscience + Ilumina miSeq')
        self.lsh_technology_comboBox.addItem('CustomArray + Ilumina miSeq')
        self.lsh_technology_comboBox.addItem('Twist Bioscience + Ilumina NextSeq')
        self.lsh_technology_comboBox.addItem('Integrated DNA Technology (IDT) + MinION')
        self.lsh_technology_comboBox.currentTextChanged.connect(self.set_lsh_chosen_clustering_technology)
    ...
    def set_lsh_q(self, value):
        self.lsh_q = value
    
    def set_lsh_sig_len(self, value):
        self.lsh_sig_len = value
    
    def set_lsh_perm_num(self, value):
        self.lsh_perm_num = value
    
    def set_lsh_iters(self, value):
        self.lsh_iters = value
    
    def set_lsh_distance_threshold(self, value):
        self.lsh_distance_threshold = value
    ...
    def set_clustering_algo(self):
        ...
        elif self.clustering_algo == 'LSH Based Algorithm':
    ...
    def set_lsh_chosen_clustering_technology(self):
        clustering_tech = self.lsh_technology_comboBox.currentText()
        self.set_chosen_clustering_technology(clustering_tech)
    ...
    def set_lsh_cluster_values(self):
        self.lsh_q_spinBox.setValue(self.lsh_q)
        self.lsh_sig_len_spinBox.setValue(self.lsh_sig_len)
        self.lsh_perm_num_spinBox.setValue(self.lsh_perm_num)
        self.lsh_iters_spinBox.setValue(self.lsh_iters)
        self.lsh_distance_threshold_spinBox.setValue(self.lsh_distance_threshold)
    ...
    def runClustering(self):
        if self.clustering_algo == 'Pseudo Clustering Algorithm':
            ...
            print(self.chosen_technology)
            self.evyat_path, self.shuffled_path = self.evyat_files_path()
            self.cluster_worker = LSHBasedClusteringWorker(self.chosen_technology, int(self.lsh_q), int(self.lsh_sig_len), int(self.lsh_perm_num), int(self.lsh_iters), int(self.lsh_distance_threshold))
            self.cluster_resutls_file = self.cluster_worker.get_results_file()
            self.progressBar.setValue(0)
            self.cluster_worker.start()
            self.label_progress.setText('Clustering in progress, please wait!')
...
class LSHBasedClusteringWorker(QThread):
    update_progress = pyqtSignal(int)

    def __init__(self, cluster_tech, lsh_q, lsh_sig_len, lsh_perm_num, lsh_iters, lsh_distance_threshold):
        super(LSHBasedClusteringWorker, self).__init__()
        self.cluster_tech = cluster_tech
        self.q = lsh_q
        self.sig_len = lsh_sig_len
        self.perm_num = lsh_perm_num
        self.iters = lsh_iters
        self.distance_threshold = lsh_distance_threshold
        if platform.system() == "Linux":
            self.results_f = '/home_nfs/sgamer8/DNAindex' + str(self.cluster_index) + '/cluster_output/' + self.cluster_tech + '/' + str(
                self.cluster_index) + '_final_results.txt'
        elif platform.system() == "Windows":
            self.results_f = 'cluster_output/' + self.cluster_tech + '/' + str('LSH') + '/' +  '_final_results.txt'

        self.directory = 'cluster_output/' + self.cluster_tech + '/' + str('LSH')
        if not os.path.exists(self.directory):
            os.makedirs(self.directory)
    ...
    def run(self):
        cluster = LSHBasedCluster(chosen_technology=self.cluster_tech, q=self.q, k=self.sig_len, m=self.perm_num, L=self.iters, distance_threshold=self.distance_threshold, report_func=self.report_func)
        result_print = cluster.run()
        # [num_of_errors, num_of_false_negative] = '44', '33' #clusters.compare_evyat_with_clustering()
        with open(self.results_f, 'w') as results_file:
            print(result_print, file=results_file)
