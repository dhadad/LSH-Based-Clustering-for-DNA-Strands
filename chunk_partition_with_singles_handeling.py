...
class LSHCluster:
    def __init__(self, all_reads, q, k, m, L, avg_clstr=27):
        """
    ...
    def chunk_paritioning(self, repeats=22):
        """
        kt = 5
        Lt= 64
        base = [(self.top ** i) for i in range(kt)]
        for itr in range(Lt):
            time_start = time.time()
            sigs = list()
            indexes = random.sample(range(self.m), kt)
            for idx in range(len(self.all_reads)):
                sig = sum(int(self.lsh_sigs[idx][indexes[i]]) * base[i] for i in range(kt))
                sigs.append((idx, sig))
            sigs.sort(key=lambda x: x[1])
            for a in range(1, len(sigs) - 1):
                if sigs[a][1] == sigs[a + 1][1]:
                    p1 = self.rep_find(sigs[a + 1][0], chunks=True)
                    p2 = self.rep_find(sigs[a][0], chunks=True)
                    if p1 != p2:
                        center, merged = min(p1, p2), max(p1, p2)
        ...
        for itr in range(repeats):
            ...
            common_substr_hash = []
            for chunk in self.chunks:       # take 3 representatives from each chunk
                if len(chunk) == 0:
                    continue
                elif len(chunk) <= 5:
                    for idx in chunk:
            ...
            for idx in range(0, len(common_substr_hash) - 1, 1):
                if common_substr_hash[idx][1] == common_substr_hash[idx + 1][1]:
                    ...
                    if p1 != p2:
                        ...
                        self.chunks[merged] = []
                        self.chunk_parent[merged] = center
            print("iteration {} took: {}".format(itr, time.time() - time_itr))
        """
        # make chunks out of the singles
        time_itr = time.time()
        avg_chunk = mean([len(self.chunks[idx]) for idx in range(len(self.all_reads)) if len(self.chunks[idx]) > 1])
        singles = [idx for idx in range(len(self.all_reads)) if len(self.chunks[idx]) == 1]
        chunk = []
        ref = singles[0]
        for single in singles:
            if len(chunk) > avg_chunk:
                chunk = []
                ref = single
            else:
                p1 = self.rep_find(single, chunks=True)
                ...
                if p1 != p2:
                    ...
                    self.chunks[merged] = []
                    self.chunk_parent[merged] = center
        print("handeling singles {} took: {}".format(itr, time.time() - time_itr))
        """
        ...
        time_itr = time.time()
        avg_chunk = mean([len(self.chunks[idx]) for idx in range(len(self.all_reads)) if len(self.chunks[idx]) > 1])
        print("avg chunk size: {} (before handeling small chunks)".format(avg_chunk))
        singles = [idx for idx in range(len(self.all_reads)) if len(self.chunks[idx]) == 1]
        cntr = 0
        ref = singles[0]
        for single in singles:
            if len(chunk) > avg_chunk:
                cntr = len(self.chunks[single])
                ref = single
            else:
                p1 = self.rep_find(single, chunks=True)
                p2 = self.rep_find(ref, chunks=True)
    ...
    def static_chunks(self):
        ...
        num_chunks = 100 * num_clstrs
        chunk_size = max(int(len(self.all_reads) / num_chunks), 1)
        print("avg clstr: {} num clstrs: {} num chunks: {} chunk_size: {}".format(self.avg_clstr, num_clstrs, num_chunks, chunk_size))
        cntr = 0
    ...
    def check_all_chunks_goodness(self):
        ...
        print("total number of chunks:", sum([1 for rep in range(len(self.chunks)) if len(self.chunks[rep]) > 0]))
        print("number of chunks with size 1: {}".format(sum([1 for rep in range(len(self.chunks)) if len(self.chunks[rep]) == 1])))
    ...
    def reduced_clustering_chunk_dedicated(self, chunk_rep):
        ...
        tot = time.time()
        initial_singles = sum([1 for item in chunk if len(self.C_til[item]) == 1])
        singles_round_end = initial_singles
        r = -1
        bad_rounds = 0
        iters_num = max(MIN_REDUCED_ITERS, int(REDUCED_ITERS_FOR_LINE * len(chunk)))
    ...
    def run(self, accrcy=True):
        ...
        for chunk_rep in range(len(self.chunks)):
            if len(self.chunks[chunk_rep]) > 0 and len(self.chunks[chunk_rep]) > MIN_CHUNK_SIZE:
                self.lsh_clustering_new_draft_chunk_dedicated(chunk_rep)
                self.reduced_clustering_chunk_dedicated(chunk_rep) # consider to remove, doesn't help here
        print("time for work done in chunks: {}".format(time.time() - lsh_cls_time))
...
if __name__ == '__main__':
    ...
    lsh = LSHCluster(reads_err, q=6, k=3, m=38, L=23)
